...
!function(document, uses, requestAnimationFrame, CACHE, LTEIE8, IE9TO11) {
    function embed(svg, g) {
        if (g) {
            ...
            for (viewBox && svg.setAttribute("viewBox", viewBox); clone.childNodes.length; ) fragment.appendChild(clone.childNodes[0]);
    ...
    function onframe() {
        for (var use; use = uses[0]; ) if (LTEIE8) {
            var src, q, img = new Image();
            src = use.getAttribute("xlink:href"), q = (/\?[^#]+/.exec(src) || [ "" ])[0], 
            ...
            if (svg.removeChild(use), url_root.length) {
                ...
                xhr.s || (xhr.s = [], xhr.open("GET", url_root), xhr.onload = onload, 
                xhr.send()), xhr.s.push([ svg, url_hash ]), 4 === xhr.readyState && xhr.onload();
            } else embed(svg, document.getElementById(url_hash));
        }
        requestAnimationFrame(onframe);
    }
    (LTEIE8 || IE9TO11) && onframe();
